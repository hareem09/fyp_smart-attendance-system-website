...
function EnrollmentTab({ students, onReject, onApprove, onRefresh }) {
  ...
  // Initialize local students from props — only pending / not_enrolled
  useEffect(() => {
  ...
  // Fetch approved students on mount
  useEffect(() => {
    fetchEnrollStudents();
  }, []);
  ...
  const fetchEnrollStudents = async () => {
    try {
      const res = await API.get('/admin/users?role=student&enrollmentStatus=approved');
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      fetchEnrollStudents(); // refresh approved list so the student appears there
    ...
    <div className="space-y-6">
      ...
      <div>
        <p className="text-sm text-gray-500 mb-3">
          <span className="font-medium text-gray-800">{filtered.length}</span> pending enrollments
        </p>
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={loadingApprove === student._id || loadingReject === student._id}
                  ...
                  >
                    {loadingApprove === student._id ? '⏳ Approving...' : '✅ Approve'}
                  </button>
                  <button
                    onClick={() => handleReject(student._id)}
                    disabled={loadingReject === student._id || loadingApprove === student._id}
                  ...
                  >
                    {loadingReject === student._id ? '⏳ Rejecting...' : '❌ Reject'}
                  </button>
                </div>
              </div>
            ))}
      ...
      {filteredApproved.length > 0 && (
        <div>
          <p className="text-sm text-gray-500 mb-3">
            <span className="font-medium text-gray-800">{filteredApproved.length}</span> approved students
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredApproved.map((student, i) => (
              <div
                key={student._id || i}
              ...
              >
                ...
                {/* Approved Badge — no action buttons, just status */}
